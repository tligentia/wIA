# wIA — Multi-Engine AI Chat Interface (v2607.AZ)
...
- 🌍 **Bilingüe ES/EN + documentación regenerada (v2607.AZ)**: wIA habla ahora **español e inglés**. **Detecta el idioma del navegador** y permite fijarlo manualmente en *Ajustes → Experiencia → Idioma* (se memoriza y tiene prioridad). Motor i18n por atributos (`data-i18n`, `-html`, `-title`, `-ph`) que traduce interfaz, navegación, paneles y textos de ayuda. La **documentación se ha regenerado por completo en ambos idiomas** (19 secciones) e incorpora todo lo nuevo: selectores rápidos `/` `//` `///`, fallback ante fallo o créditos agotados, aceleración y precarga de WebGPU, salvaguarda de móvil, anonimización DLP con +20 tipos y sus conmutadores, temas (incl. Tligent) e idioma, cola persistente y copias de seguridad.
